...
class ModernAttendanceSystem:
    ...
    def detect_emotion(self, face_roi):
        try:
            ...
            smiles = self.smile_cascade.detectMultiScale(gray, scaleFactor=1.7, minNeighbors=20, minSize=(int(w*0.2), int(h*0.1)))
            eyes = self.eye_cascade.detectMultiScale(gray, 1.1, 6, minSize=(int(w*0.08), int(h*0.05)))
            ...
            for (ex, ey, ew, eh) in eyes:
                eye_area += ew * eh
            face_area = max(1, w * h)
            eye_ratio = eye_area / face_area
            if eye_ratio > 0.03 and len(eyes) >= 2:
                return 'surprised'
        except:
            pass
        return 'neutral'
    ...
    def enrollment_ui(self):
        ...
        if emp_id in self.employees:
            print(f"  ⚠ Employee '{emp_id}' is already enrolled")
            print("  This face has already been registered in the system.")
            time.sleep(2)
            return
        ...
        while time.time() - start < timeout:
            ...
            if len(faces) == 1:
                ...
                if emp_id:
                    ...
                    cv2.putText(disp, f"Emotion: {emotion}", (nx+15, info_y+110),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.colors['text_light'], 1, cv2.LINE_AA)
