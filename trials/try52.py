...
class EmotionDetector:
    ...
    def __init__(self):
        ...
        # --- OPTIMIZED HEURISTIC THRESHOLDS BASED ON USER DATA ---
        # 1. Stress: Set above the measured noise baseline (0.28). 
        # Only classify as stressed if tension is significantly higher than neutral texture.
        self.NORMALIZED_STRESS_THRESH = 0.38
        
        # 2. Happy: Raised slightly to demand a more pronounced smile/cheeks.
        self.HIGH_HAPPY_RATIO_THRESH = 1.18
        
        # 3. Sad: Lowered to require lower face to be noticeably darker.
        self.LOW_SAD_RATIO_THRESH = 0.85
        
        # 4. Sad Variance: Max total variance allowed for Sad/Flat face.
        self.LOW_VAR_THRESH = 950
        
        
    def detect_emotion(self, face_roi):
        """Revised emotion detection logic with refined sequential ordering and optimized thresholds."""
        ...
        try:
            ...
            # 2. Variance Calculation (Tension/Texture)
            total_variance = np.var(gray)
            normalized_variance = total_variance / (mean_upper**2 + 1e-6)
            
            emotion = 'neutral'  # Default

            # --- Sequential Decision Logic (Optimal Order) ---
            
            # 1. Stressed: Priority check for high tension (normalized variance).
            if normalized_variance > self.NORMALIZED_STRESS_THRESH:
                emotion = 'stressed'
            
            # 2. Sad: Check for extreme lack of expression (low ratio AND low total variance).
            elif brightness_ratio < self.LOW_SAD_RATIO_THRESH and total_variance < self.LOW_VAR_THRESH:
                 emotion = 'sad'
            
            # 3. Happy: Check for pronounced smile (high ratio).
            elif brightness_ratio > self.HIGH_HAPPY_RATIO_THRESH:
                emotion = 'happy'
            
            # 4. Neutral: The fallback for all intermediate states.
            else:
                emotion = 'neutral'
    ...
    def get_message(self, emotion):
        """Get random message for detected emotion (Preserved)."""
        return np.random.choice(self.emotions[emotion]['messages'])

    def get_emoji(self, emotion):
        """Get emoji for detected emotion (Preserved)."""
...
class AntiSpoofing:
    ...
    def __init__(self):
        # Reduced maxlen for faster variance calculation
        self.motion_history = deque(maxlen=5) # Reduced from 15
        self.texture_history = deque(maxlen=3) # Reduced from 10
        self.prev_gray = None # For fast optical flow-like motion
        self.prev_eye_brightness = 0.0 # For fast blink proxy
    ...
    def reset(self):
        ...
        self.prev_gray = None
        self.prev_eye_brightness = 0.0
    
    def detect_motion(self, face_roi):
        """Detect natural facial micro-movements using simple frame difference."""
        try:
            ...
            # Fast motion detection: Calculate the difference between the current and previous frame
            diff = cv2.absdiff(current_gray, self.prev_gray)
            _, thresh = cv2.threshold(diff, 20, 255, cv2.THRESH_BINARY)
            
            # Use total percentage of movement/difference
            motion_percentage = np.sum(thresh) / (thresh.size * 255)
            
            self.prev_gray = current_gray # Update previous frame
            
            # If motion is above a tiny noise threshold, it's natural movement
            return motion_percentage > 0.005 # Tiny threshold for micro-movements
        except:
            self.prev_gray = None
            return False
    
    def detect_blink(self, face_roi):
        """Fast blink detection using eye region brightness proxy."""
        try:
            gray = cv2.cvtColor(face_roi, cv2.COLOR_BGR2GRAY)
            h, w = gray.shape
            
            # Analyze central eye region (e.g., top 35% of face)
            eye_region = gray[:int(h*0.35), int(w*0.1):int(w*0.9)]
            current_brightness = np.mean(eye_region)
            
            # Simple, fast proxy: check for a sudden, large drop in eye brightness
            # Only track change if it's a significant drop (potential eye closure)
            if self.prev_eye_brightness > 0 and current_brightness < self.prev_eye_brightness * 0.75:
                # If brightness dropped 25%, count as a blink proxy
                self.prev_eye_brightness = current_brightness
                return True
    ...
    def analyze_texture(self, face_roi):
        """Analyze texture patterns (photos have different texture than real skin)"""
        try:
            # Use a pre-resized face if available to save time
            resized_face = cv2.resize(face_roi, (40, 40)) # Tiny size for fast calculation
            gray = cv2.cvtColor(resized_face, cv2.COLOR_BGR2GRAY)
            
            # Calculate local binary pattern-like feature (Laplacian is fast)
            ...
            if len(self.texture_history) >= 3: # Check faster
                avg_texture = np.mean(list(self.texture_history))
                return avg_texture > 120  # Raised threshold for better photo rejection
            return False
        except:
            return False
...
class ModernAttendanceSystem:
    ...
    def __init__(self):
        ...
        self.last_face_rect = None  # Stores (x, y, w, h) for ROI tracking
        self.full_scan_interval = 5 # Scan full frame every 5th frame
        self.frame_count = 0
    ...
    def check_ui(self, action):
        ...
        while time.time() - start < timeout:
            ...
            # Perform full scan only every 'full_scan_interval' frames
            perform_full_scan = self.frame_count % self.full_scan_interval == 0
            
            if perform_full_scan or self.last_face_rect is None:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                # Reduced scaleFactor from 1.1 to 1.2 for speed, kept minNeighbors at 5
                faces = self.face_cascade.detectMultiScale(gray, 1.2, 5, minSize=(100,100)) 
                
                # If face detected, update last known rectangle
                if len(faces) == 1:
                    self.last_face_rect = faces[0]
            
            elif self.last_face_rect is not None:
                # ROI Tracking: Search only around the previous face location
                x, y, w, h = self.last_face_rect
                # Define search area with a margin (e.g., 20%)
                margin = int(w * 0.2)
                x1 = max(0, x - margin)
                y1 = max(0, y - margin)
                ...
                # Crop the ROI to search
                roi_frame = frame[y1:y2, x1:x2]
                gray_roi = cv2.cvtColor(roi_frame, cv2.COLOR_BGR2GRAY)

                # Detect faces only in the ROI
                roi_faces = self.face_cascade.detectMultiScale(gray_roi, 1.2, 5, minSize=(100,100))

                if len(roi_faces) == 1:
                    rx, ry, rw, rh = roi_faces[0]
                    # Map ROI coordinates back to full frame
                    x, y, w, h = rx + x1, ry + y1, rw, rh
                    faces = [(x, y, w, h)]
                    self.last_face_rect = faces[0]
                else:
                    # If lost in ROI, reset for a full scan on the next frame
                    self.last_face_rect = None
    ...
    def check_ui(self, action):
        ...
        while time.time() - start < timeout:
            ...
            if len(faces) == 1:
                ...
                # Only proceed if liveness check passes
                if liveness_frames >= required_frames:
                    ...
                    if emp_id:
                        ...
                        # Detect emotion
                        emotion = 'neutral'
                        # print(self.config.get('emotion_detection_enabled', True))
