...
class AntiSpoofing:
    ...
    def detect_motion(self, face_roi):
        """Detect natural facial micro-movements"""
        try:
            gray = cv2.cvtColor(face_roi, cv2.COLOR_BGR2GRAY)
            gray = cv2.GaussianBlur(gray, (5, 5), 0)
            
            # Calculate image moment (helps detect small movements)
            ...
            # Check for natural movement variance
            if len(self.motion_history) >= 10:
                positions = np.array(list(self.motion_history))
                variance = np.var(positions, axis=0).sum()
                return variance > 5  # Natural faces have micro-movements
            return False
        except:
            return False
    
    def detect_blink(self, face_roi):
        """Simple blink detection using eye region analysis"""
        try:
            gray = cv2.cvtColor(face_roi, cv2.COLOR_BGR2GRAY)
            h, w = gray.shape
            
            # Analyze eye regions (upper 40% of face)
            ...
            # Check for blink pattern (sudden increase then decrease)
            if len(self.blink_history) >= 20:
                recent = list(self.blink_history)[-20:]
                peaks = []
                for i in range(1, len(recent)-1):
                    if recent[i] > recent[i-1] and recent[i] > recent[i+1]:
                        if recent[i] > 0.15:  # Significant closure
                            peaks.append(i)
                return len(peaks) >= 1  # At least one blink detected
            return False
        except:
            return False
    ...
    def analyze_texture(self, face_roi):
        ...
        try:
            ...
            if len(self.texture_history) >= 5:
                ...
                return avg_texture > 100  # Real skin has more texture detail
            return False
        except:
            return False
